...
<main id="primary" class="site-main">
    <!-- Hero Section -->
    <section class="relative min-h-[80vh] bg-gradient-to-br from-white via-blue-50/30 to-purple-50/20">
        <!-- Background Pattern -->
        <div class="absolute inset-0 bg-grid-pattern opacity-[0.02]"></div>

        <!-- Floating Elements -->
        <div class="absolute top-20 left-10 w-20 h-20 bg-blue-500/10 rounded-full blur-xl animate-pulse"></div>
        <div class="absolute top-40 right-20 w-32 h-32 bg-purple-500/10 rounded-full blur-xl animate-pulse delay-1000"></div>
        <div class="absolute bottom-40 left-1/4 w-24 h-24 bg-green-500/10 rounded-full blur-xl animate-pulse delay-500"></div>
            ...
            <div class="text-center space-y-8">
                        ...
                        <div class="relative bg-white rounded-2xl shadow-2xl p-2 border border-gray-200/50">
                            <form method="get" action="<?php echo esc_url(home_url('/')); ?>" class="flex items-center">
                                ...
                                <input type="text" name="s" 
                                       ...
                                       class="flex-1 px-4 py-4 text-lg bg-transparent border-0 focus:outline-none focus:ring-0" />
                                <button type="submit" class="m-1 px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-medium rounded-xl transition-all">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <a href="<?php echo esc_url(get_post_type_archive_link('floor_plan')); ?>" 
                       class="inline-flex items-center px-8 py-3 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition-colors">
                        <?php _e('Browse All Categories', 'artsplans'); ?>
                    </a>
                    <a href="<?php echo esc_url(wp_registration_url()); ?>" 
                       class="inline-flex items-center px-8 py-3 bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700 text-white rounded-lg transition-all">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-3xl mx-auto pt-12">
                    ...
                    $stats = array(
                        array(
                            'value' => wp_count_posts('floor_plan')->publish . '+',
                            'label' => __('Floor Plans', 'artsplans'),
                            'icon' => 'M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10'
                        ),
                        array(
                            'value' => wp_count_posts('model_3d')->publish . '+',
                ...
                if ($categories && !is_wp_error($categories)):
                    foreach (array_slice($categories, 0, 8) as $category): ?>
                        <a href="<?php echo esc_url(get_term_link($category)); ?>" 
                           class="group bg-gray-50 rounded-xl p-6 text-center hover:shadow-lg transition-all card-hover">
                            ...
                            <h3 class="font-semibold text-gray-900 mb-2"><?php echo esc_html($category->name); ?></h3>
